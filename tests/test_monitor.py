...
class TestSleep(TestCase):
    def setUp(self):
        ...
        self.mock_DHT = MagicMock()
        self.mock_dht.DHT.return_value = self.mock_DHT
    ...
    def test_init(self):
        with self.module_patcher:
            ...
            self.mock_machine.Pin.assert_called_with(
                config['dht_pin'])
            assert self.mock_dht.DHT.called
    ...
    def test_sample(self):
        with self.module_patcher:
            ...
            self.mock_DHT.temperature.return_value = expected_tmp
            self.mock_DHT.humidity.return_value = expected_hum
            ...
            assert self.mock_DHT.measure.called
